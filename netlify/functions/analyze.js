...
出力形式:
{
  ...
  "reincarnations": [
    ...
    // 固定のフォールバックレスポンス（OpenAI API呼び出しをスキップ）
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({
        destiny: `${name}さんは${mbti}タイプの持ち主で、${birthdate}生まれの特性から、新たな価値を創造し、周囲の人々に影響を与える天命を持っています。独自の視点と創造性を活かして、世界に変化をもたらす可能性を秘めています。`,
        reincarnations: [
          {
            name: "レオナルド・ダ・ヴィンチ",
            years: "1452-1519",
    ...
    /* 以下のOpenAI API呼び出しは現在のエラーを回避するため、コメントアウトしています
    // API呼び出し
